<x-app-layout>
    ...
    <div class="py-12 relative">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            ...
            {{-- MENSAGEM DE SUCESSO (Opcional, mas bom manter padrão) --}}
            @if(session('status'))
            ...
            <div class="glass-card rounded-2xl shadow-xl overflow-hidden mb-6">
                <div class="p-4 bg-white/70 backdrop-blur-sm border border-white/40 flex flex-col lg:flex-row justify-between items-center gap-4">
                    ...
                    <form action="{{ route('admin.produtos.index') }}" method="GET" class="flex flex-col md:flex-row gap-4 w-full lg:flex-1">
                        ...
                        <div class="flex gap-2">
                            ...
                            @if(request('search') || request('tipo'))
                                <a href="{{ route('admin.produtos.index') }}" class="px-4 py-2 text-sm text-gray-500 hover:text-[#7B19E5] transition-colors flex items-center">
                                    Limpar
                                </a>
                            @endif
                        </div>
                    ...
                    <div class="w-full lg:w-auto flex justify-end shrink-0 border-t lg:border-t-0 lg:border-l border-[#FFD6F4] pt-4 lg:pt-0 lg:pl-4">
                        <a href="{{ route('admin.produtos.criar') }}" class="flex items-center gap-2 bg-white text-[#7B19E5] border-2 border-[#7B19E5] px-6 py-2.5 rounded-full font-bold hover:bg-[#7B19E5] hover:text-white transition-all shadow-sm whitespace-nowrap">
                                    ...
                                    <tr class="hover:bg-white/50 transition-colors">
                                        ...
                                        <td class="px-6 py-4 whitespace-nowrap">
                                            ...
                                            @if($produto->quantidade_estoque <= 5)
                                                <span class="ml-2 text-[10px] bg-red-100 text-red-600 px-2 py-0.5 rounded-full font-bold uppercase tracking-wider">Baixo</span>
                                            @endif
                                        </td>
                                ...
                                @empty
                                    <tr>
                                        <td colspan="5" class="px-6 py-8 text-center text-gray-500">
                                            ...
                                            @if(request('search') || request('tipo'))
                                                <br><span class="text-xs">Tente outra busca ou <a href="{{ route('admin.produtos.index') }}" class="text-[#7B19E5]">limpe o filtro</a></span>
                                            @else
                                                <br><span class="text-xs">Clique em "Novo Produto" para começar</span>
                                            @endif
                                        </td>
                                    </tr>
                                @endforelse
