                ...
                <div class="p-6 bg-white/70 backdrop-blur-sm border border-white/40">
                    ...
                    <div class="flex items-center gap-3 mb-6">
                        ...
                        <h2 class="text-xl font-title text-[#4A00B9]">
                            {{ __('Editar Serviço: ') }} <span class="text-[#FF2EB6]">{{ $servico->nome }}</span>
                        </h2>
                    </div>
...
{{ __('Nome do Serviço') }} <span class="text-[#FF2EB6]">*</span>
                            ...
                            @else
                                <input value="{{ $servico->nome }}" 
                                    class="w-full px-4 py-3 bg-gray-100/50 border border-[#FFD6F4] rounded-lg text-gray-500 cursor-not-allowed" readonly />
...
{{ __('Preço (R$)') }} <span class="text-[#FF2EB6]">*</span>
                            ...
                            @else
                                <input value="R$ {{ number_format($servico->preco, 2, ',', '.') }}" 
                                    class="w-full px-4 py-3 bg-gray-100/50 border border-[#FFD6F4] rounded-lg text-gray-500 cursor-not-allowed" readonly />
...
{{ __('Duração (minutos)') }} <span class="text-[#FF2EB6]">*</span>
                            ...
                            @else
                                <input value="{{ $servico->duracao }} minutos" 
                                    class="w-full px-4 py-3 bg-gray-100/50 border border-[#FFD6F4] rounded-lg text-gray-500 cursor-not-allowed" readonly />
                                <input type="hidden" name="duracao" value="{{ $servico->duracao }}">
                            @endif
