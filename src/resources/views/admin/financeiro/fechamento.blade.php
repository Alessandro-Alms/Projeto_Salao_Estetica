        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
                ...
                <div class="flex gap-3 flex-wrap">
                    <form action="{{ route('admin.financeiro.fechamento') }}" method="GET" class="glass-card rounded-2xl overflow-hidden">
                        <div class="flex items-center gap-2 p-2 bg-white/70 backdrop-blur-sm border border-white/40">
                            <label for="data" class="text-sm text-[#4A00B9] font-medium ml-2">
                                ✧ Mudar data:
                            </label>
                    ...
                    <a href="{{ route('admin.financeiro.fechamento.pdf', ['data' => $dataSelecionada]) }}" class="bg-gradient-to-r from-[#7B19E5] to-[#FF2EB6] text-white px-5 py-2 rounded-full font-medium btn-primary shadow-md hover:shadow-lg transition-all text-sm inline-flex items-center gap-2">
                        ✧ Baixar PDF
                    </a>
                </div>
            </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                ...
                <div class="glass-card rounded-2xl shadow-xl overflow-hidden">
                    <div class="p-6 bg-white/70 backdrop-blur-sm border border-white/40">
                        <h3 class="text-gray-500 font-bold uppercase text-xs mb-2">Total Comissões (Saída)</h3>
                        <p class="text-3xl font-black text-red-500">- R$ {{ number_format($totalComissoes, 2, ',', '.') }}</p>
                    </div>
                </div>
                ...
                <div class="glass-card rounded-2xl shadow-xl overflow-hidden">
                    <div class="p-6 bg-white/70 backdrop-blur-sm border border-white/40">
                        <h3 class="text-gray-500 font-bold uppercase text-xs mb-2">Lucro Líquido Salão</h3>
                        <p class="text-3xl font-black text-green-600">R$ {{ number_format($lucroLiquido, 2, ',', '.') }}</p>
                    </div>
                </div>
            </div>
            ...
            <div class="glass-card rounded-2xl shadow-xl overflow-hidden mb-6">
                <div class="p-5 bg-white/70 backdrop-blur-sm border-b border-[#FFD6F4]">
                    <div class="flex items-center gap-2">
                        <span class="text-[#7B19E5]">✧</span>
                        <h3 class="text-lg font-title text-[#4A00B9]">Detalhamento Financeiro do Dia</h3>
                    </div>
                </div>
                <div class="p-6 bg-white/70 backdrop-blur-sm">
                    <!-- Faturamento Bruto -->
                    <div class="mb-6 pb-6 border-b border-[#FFD6F4]">
                        <h4 class="text-gray-500 font-bold uppercase text-xs mb-3">Faturamento bruto</h4>
                        <div class="grid grid-cols-1 md:grid-cols-4 gap-3">
                            ...
                            <div class="p-4 bg-amber-50/50 rounded-lg border border-amber-200">
                                ...
                                <p class="text-lg font-black text-amber-600">R$ {{ number_format($totalMultas, 2, ',', '.') }}</p>
                            </div>
                        </div>
                    ...
                    <div class="mb-6 pb-6 border-b border-[#FFD6F4]">
                        <h4 class="text-gray-500 font-bold uppercase text-xs mb-3">Comissões a pagar</h4>
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-3">
                            <div class="p-4 bg-red-50/50 rounded-lg border border-red-200">
                                ...
                                <p class="text-lg font-black text-red-600">R$ {{ number_format($totalComissoesServicos, 2, ',', '.') }}</p>
                            ...
                            <div class="p-4 bg-orange-50/50 rounded-lg border border-orange-200">
                                ...
                                <p class="text-lg font-black text-orange-600">R$ {{ number_format($totalComissoesProdutos, 2, ',', '.') }}</p>
                    ...
                    <div class="mb-6">
                        <h4 class="text-gray-500 font-bold uppercase text-xs mb-3">Resultado líquido</h4>
                        <div class="p-4 bg-green-50/80 rounded-lg border-2 border-green-300">
                            <div class="flex justify-between items-center">
                                ...
                                <span class="text-3xl font-black text-green-600">R$ {{ number_format($lucroLiquido, 2, ',', '.') }}</span>
                            </div>
                    ...
                    <div class="pt-4 border-t border-[#FFD6F4]">
                        <h4 class="text-gray-500 font-bold uppercase text-xs mb-3">Resumo do dia</h4>
                        <div class="grid grid-cols-2 md:grid-cols-4 gap-3">
                            <div class="p-3 bg-white/50 rounded-lg border border-[#FFD6F4] text-center">
                                ...
                                <p class="text-2xl font-black text-[#7B19E5]">{{ $agendamentos->count() }}</p>
                            </div>
                            @if($agendamentos->count() > 0)
                            <div class="p-3 bg-white/50 rounded-lg border border-[#FFD6F4] text-center">
                                <p class="text-xs text-gray-600 mb-1">Ticket Médio</p>
                                <p class="text-2xl font-black text-[#7B19E5]">R$ {{ number_format($agendamentos->count() > 0 ? $totalServicos / $agendamentos->count() : 0, 2, ',', '.') }}</p>
                            </div>
                            @endif
                            <div class="p-3 bg-white/50 rounded-lg border border-[#FFD6F4] text-center">
                                <p class="text-xs text-gray-600 mb-1">Total Vendas</p>
                                <p class="text-2xl font-black text-[#7B19E5]">{{ $vendas->count() }}</p>
                            </div>
                            <div class="p-3 bg-white/50 rounded-lg border border-[#FFD6F4] text-center">
                                <p class="text-xs text-gray-600 mb-1">% Comissão</p>
                                <p class="text-2xl font-black text-[#7B19E5]">{{ ($totalServicos + $totalProdutos + $totalPacotes + $totalMultas > 0) ? number_format(($totalComissoes / ($totalServicos + $totalProdutos + $totalPacotes + $totalMultas)) * 100, 1, ',', '.') : '0' }}%</p>
                            </div>
                        </div>
                    </div>
                </div>
