@php
    $clienteId = auth()->id();
    $agendamentoProximo = \App\Models\Agendamento::where('cliente_id', $clienteId)
        ->whereIn('status', ['confirmado', 'presente'])
        ->where('data_hora_inicio', '>', now())
        ->orderBy('data_hora_inicio')
        ->first();

    $avaliacoesPendentes = \App\Models\Agendamento::where('cliente_id', $clienteId)
        ->where('status', 'executado')
        ->doesntHave('avaliacao')
        ->count();

    $pacotesVencendo = \App\Models\ClientePacote::where('cliente_id', $clienteId)
        ->where('status', 'ativo')
    ...
    if ($agendamentoProximo) {
        $inicio = \Carbon\Carbon::parse($agendamentoProximo->data_hora_inicio);
