...
<div class="container mx-auto p-4 max-w-6xl">
    <div class="glass-card rounded-2xl shadow-xl overflow-hidden p-8">
        ...
        {{-- Indicador de Passos --}}
        <div class="flex justify-between mb-8 border-b border-[#FFD6F4] pb-4 flex-wrap gap-2">
            <div id="indicador-1" class="font-bold text-[#7B19E5] text-sm md:text-base">✧ Serviço</div>
            <div id="indicador-2" class="text-gray-400 text-sm md:text-base">✦ Calendário</div>
            <div id="indicador-3" class="text-gray-400 text-sm md:text-base">✧ Horário</div>
            <div id="indicador-4" class="text-gray-400 text-sm md:text-base">✦ Profissional</div>
            <div id="indicador-5" class="text-gray-400 text-sm md:text-base">✧ Confirmar</div>
        </div>
            ...
            <div id="passo-1" class="passo">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                    @foreach($servicos as $servico)
                        <div class="flex items-start p-5 border-2 border-[#FFD6F4] rounded-lg cursor-pointer hover:border-[#7B19E5] hover:bg-white/50 transition servico-card" 
                            ...
                            <div class="w-full">
                                <p class="font-bold text-lg text-[#1A002B]">{{ $servico->nome }}</p>
                                <p class="text-sm text-gray-600 mt-2">R$ {{ number_format($servico->preco, 2, ',', '.') }}</p>
                                <p class="text-xs text-gray-500 mt-2">✧ Duração: {{ $servico->duracao }} min</p>
                            </div>
                        </div>
                    @endforeach
                </div>
                ...
                <div id="resumo-servicos-selecionados" class="mt-8 p-4 bg-[#7B19E5]/5 border-2 border-[#FFD6F4] rounded-lg hidden">
                    ...
                    <div class="mt-4 pt-4 border-t-2 border-[#FFD6F4]">
                        <p class="font-bold text-[#4A00B9]">✧ Tempo Total: <span id="tempo-total">0</span> minutos</p>
                    </div>
                </div>
                
                <div class="mt-12 flex justify-end">
                    <button type="button" onclick="irParaPasso(2)" class="bg-gradient-to-r from-[#7B19E5] to-[#FF2EB6] text-white px-8 py-3 rounded-full font-medium btn-primary shadow-md hover:shadow-lg transition">Próximo →</button>
                </div>
            </div>

            {{-- PASSO 2: CALENDÁRIO --}}
            <div id="passo-2" class="passo hidden">
                <h3 class="text-2xl font-title text-[#4A00B9] mb-8">Seleciona uma data</h3>
                
                <div class="bg-white/30 p-8 rounded-lg border-2 border-[#FFD6F4]">
                    <div id="calendario" class="calendar-container"></div>
                </div>

                <div id="info-data-selecionada" class="mt-6 p-4 bg-green-50/80 border-2 border-green-200 rounded-lg hidden">
                ...
                <div class="mt-12 flex justify-between">
                    <button type="button" onclick="irParaPasso(1)" class="bg-gray-200 px-8 py-3 rounded-full font-medium hover:bg-gray-300 transition">← Voltar</button>
                    <button type="button" onclick="irParaPasso(3)" class="bg-gradient-to-r from-[#7B19E5] to-[#FF2EB6] text-white px-8 py-3 rounded-full font-medium btn-primary shadow-md hover:shadow-lg transition">Próximo →</button>
                </div>
            </div>

            {{-- PASSO 3: HORÁRIO --}}
            <div id="passo-3" class="passo hidden">
                <h3 class="text-2xl font-title text-[#4A00B9] mb-8">Seleciona um horário</h3>
                ...
                <div id="aviso-horario-especial" class="mt-4 p-4 bg-yellow-50/80 border-2 border-yellow-200 rounded-lg hidden">
                ...
                <div id="info-hora-selecionada" class="mt-6 p-4 bg-green-50/80 border-2 border-green-200 rounded-lg hidden">
                ...
                <div class="mt-12 flex justify-between">
                    <button type="button" onclick="irParaPasso(2)" class="bg-gray-200 px-8 py-3 rounded-full font-medium hover:bg-gray-300 transition">← Voltar</button>
                    <button type="button" onclick="irParaPasso(4)" class="bg-gradient-to-r from-[#7B19E5] to-[#FF2EB6] text-white px-8 py-3 rounded-full font-medium btn-primary shadow-md hover:shadow-lg transition">Próximo →</button>
                </div>
            </div>

            {{-- PASSO 4: PROFISSIONAL --}}
            <div id="passo-4" class="passo hidden">
                <h3 class="text-2xl font-title text-[#4A00B9] mb-8">Seleciona o profissional</h3>
                ...
                <div id="info-profissional-selecionado" class="mt-6 p-4 bg-green-50/80 border-2 border-green-200 rounded-lg hidden">
                ...
                <div class="mt-12 flex justify-between">
                    <button type="button" onclick="irParaPasso(3)" class="bg-gray-200 px-8 py-3 rounded-full font-medium hover:bg-gray-300 transition">← Voltar</button>
                    <button type="button" onclick="irParaPasso(5)" class="bg-gradient-to-r from-[#7B19E5] to-[#FF2EB6] text-white px-8 py-3 rounded-full font-medium btn-primary shadow-md hover:shadow-lg transition">Próximo →</button>
                </div>
            </div>

            {{-- PASSO 5: CONFIRMAR --}}
            <div id="passo-5" class="passo hidden">
                <h3 class="text-2xl font-title text-[#4A00B9] mb-8">Confirma os dados do teu agendamento</h3>
                
                <div class="bg-gradient-to-r from-[#7B19E5]/5 to-[#FF2EB6]/5 p-8 rounded-lg border-2 border-[#FFD6F4] space-y-4">
                    <div class="flex items-center justify-between pb-4 border-b-2 border-[#FFD6F4]">
                        <span class="text-gray-700 font-semibold">✧ Serviço:</span>
                        <span id="resumo_servico" class="font-bold text-[#4A00B9] text-lg"></span>
                    </div>
                    <div class="flex items-center justify-between pb-4 border-b-2 border-[#FFD6F4]">
                        <span class="text-gray-700 font-semibold">✦ Profissional:</span>
                        <span id="resumo_profissional" class="font-bold text-[#4A00B9] text-lg"></span>
                    </div>
                    <div class="flex items-center justify-between pb-4 border-b-2 border-[#FFD6F4]">
                        <span class="text-gray-700 font-semibold">✧ Data:</span>
                        <span id="resumo_data" class="font-bold text-[#4A00B9] text-lg"></span>
                    </div>
                    <div class="flex items-center justify-between pb-4 border-b-2 border-[#FFD6F4]">
                        <span class="text-gray-700 font-semibold">✦ Hora:</span>
                        <span id="resumo_hora" class="font-bold text-[#4A00B9] text-lg"></span>
                    </div>
                    <div class="flex items-center justify-between pt-4 border-t-2 border-[#FFD6F4]">
                        <span class="text-gray-700 font-semibold">Valor base:</span>
                        <span id="resumo_valor_base" class="font-bold text-[#7B19E5] text-lg"></span>
                    </div>
                    ...
                    <div id="linha_desconto_combo" class="flex items-center justify-between hidden">
                        ...
                        <span id="resumo_desconto_combo" class="font-bold text-green-700 text-lg"></span>
                    </div>
                    <div class="flex items-center justify-between pt-4 border-t-2 border-[#FFD6F4]">
                        <span class="text-gray-900 font-bold">Total a pagar:</span>
                        <span id="resumo_valor_total" class="font-bold text-green-600 text-2xl"></span>
                    </div>
                </div>

                <div class="mt-12 flex justify-between">
                    <button type="button" onclick="irParaPasso(4)" class="bg-gray-200 px-8 py-3 rounded-full font-medium hover:bg-gray-300 transition">← Voltar</button>
                    <button type="submit" class="bg-gradient-to-r from-[#7B19E5] to-[#FF2EB6] text-white px-12 py-3 rounded-full font-medium btn-primary shadow-md hover:shadow-lg transition text-lg">Confirmar Agendamento</button>
                </div>
            </div>
...
<style>
    ...
    .calendar-date.selecionado {
        ...
        color: white;
        border-color: #7B19E5;
        box-shadow: 0 6px 20px rgba(123, 25, 229, 0.5);
        font-weight: 700;
    }
    ...
    .profissional-card:hover {
        ...
        box-shadow: 0 10px 30px rgba(123, 25, 229, 0.2);
    ...
    .profissional-card.selecionado {
        background: rgba(123, 25, 229, 0.1);
        border-color: #7B19E5;
        box-shadow: 0 10px 30px rgba(123, 25, 229, 0.4);
...
<script>
    ...
    function toggleServico(servicoId, servicoNome, duracao, preco) {
        ...
        if (estadoAgendamento.servicosIds.includes(servicoId)) {
            ...
            card.classList.add('border-[#7B19E5]', 'border-4', 'bg-white/50');
        }
    ...
    function atualizarResumoServicos() {
        ...
        estadoAgendamento.servicosIds.forEach((id, index) => {
            ...
            li.innerHTML = `<span class="text-[#4A00B9]">✧ ${nome}</span> <span class="text-gray-600">(${duracao} min)</span>`;
    ...
    function carregarProfissionaisDisponiveis() {
        ...
        gradeProfissionais.innerHTML = '<p class="text-gray-500 col-span-full text-center py-8">Carregando profissionais...</p>';
            ...
            .then(profissionais => {
                ...
                profissionais.forEach(prof => {
                    ...
                    const avisoDesconto = prof.desconto_servicos > 0
                        ? `<p class="text-green-700 font-semibold">Desconto combo: -${formatarMoeda(prof.desconto_servicos)} (${prof.motivo_desconto})</p>`
                        : '';
                    card.innerHTML = `
                        <h4>✧ ${prof.name}</h4>
                        ...
                        ${avisoPreco}
                        ${avisoDesconto}
                        <p class="font-bold text-[#7B19E5]">Total previsto: ${formatarMoeda(prof.valor_total)}</p>
                    `;
    ...
    function irParaPasso(passo) {
        ...
        for (let i = 1; i <= 5; i++) {
            ...
            if (i === passo) {
                ...
                ind.classList.remove('font-bold', 'text-[#7B19E5]', 'text-gray-400');
            } else {
                ind.classList.remove('font-bold', 'text-[#7B19E5]', 'text-green-600', 'font-semibold');
